...
# Logging channel
LOG_CHANNEL = --1002525208664  # Replace with your actual log channel ID

cookies_file_path = os.getenv("COOKIES_FILE_PATH", "youtube_cookies.txt")

# Image URLs
cpimg = "https://picsum.photos/300"
...
OWNER_ID = 6473502061  # Replace with the actual owner's user ID

# List of sudo users (initially empty or pre-populated)
SUDO_USERS = [6473502061]

AUTH_CHANNEL = -1002662569444
...
caption = (
        ...
        "➠ **ғᴏʀ ᴜsᴇ ᴍᴇ sᴇɴᴅ /jackson.\n"
        "➠ **ғᴏʀ ɢᴜɪᴅᴇ sᴇɴᴅ /help." #comment
)
...
async def help_command(client: Client, msg: Message):
    help_text = (
        "`/start` - Start the bot⚡\n\n"
        "`/jackson` - Download and upload files (sudo)🎬\n\n"
...
# Upload command handler - Renamed from tushar to jackson
@bot.on_message(filters.command(["jackson"]))
async def upload(bot: Client, m: Message):
    if not is_authorized(m.chat.id):
        await m.reply_text("**🚫You are not authorized to use this bot.**")
        await log_to_channel(bot, f"#UNAUTHORIZED_ACCESS\nUser: {m.from_user.id} tried to access /jackson")
        return
